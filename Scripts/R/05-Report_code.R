  ...
  if(!is.null(pydeg_sub) && nrow(pydeg_sub)>0) {
    ...
    for (j.row in seq_len(nrow(cat_df))) {
        ...
        if (!is.null(pydeg_cat) && nrow(pydeg_cat) > 0) {
            ...
            plot_dir_prefix <- paste(
                sprintf("%02d",
                        as.numeric(row.names(pydeg_cat))),
                sep = "_")
            plot_dir_suffix <- paste0(
                paste(cat_label,
                      gsub("\\.", "_",
            ...
            ## Plot path
            gene_plot_location <-
                paste0("Dplots/Gene_", i.comparison, "/",
                       plot_dir_prefix, "_Gene_", plot_dir_suffix)

            peak_plot_location <-
                paste0("Dplots/Peak_", i.comparison, "/",
                       plot_dir_prefix, "_Peak_", plot_dir_suffix)
            ...
            P.dplot.link <- paste0(plot.prefix, '"',
                                   ...
                                   peak_plot_location,  "'",
                                   plot.suffix_widthP, '">',
                                   fa("chart-column", width = icon_size,
                                      height = icon_size,
                                    fill = "steelblue"),
                                 "</a>")
            ...
            pydeg_cat$category_1 <- factor(pydeg_cat$category_1,
                                           levels = c(1:4, 0))
            pydeg_cat$category_2 <- factor(pydeg_cat$category_2,
                                           levels = c(LETTERS[1:3]))
            pydeg_cat[, ":="(MorePeaks = ifelse(MorePeaks == 0,
                                                "No", "Yes"))]
            pydeg_cat[, ":="(rep_gene = ifelse(rep_gene == 0,
                                               "No", "Yes"))]
...
pydeg_dt$cat1_2 <- paste(pydeg_dt$category_1,
                         pydeg_dt$category_2, sep = "-")

## Reorder cols
pydeg_dt <- dplyr::select(pydeg_dt,
                          all_of(c(i.cols.sort[1:4], "TAIR",
                                   i.cols.sort[5:10], "PARE",
                                   i.cols.sort[11:length(i.cols.sort)],
                                   "Index")))
...
i.dt3 <- DT::datatable(dplyr::select(pydeg_dt, all_of(cols.coordinates)),
                       ...
                       options = list(
                           ...
                           columnDefs = list(list(visible = FALSE,
                                                  targets = "Comparison")),
                           rowGroup = list(
                               dataSrc = length(cols.coordinates) - 1),
                           dom = "Bfrtip",
                           buttons = c("copy", "csv", "excel"),
                           pageLength = 150)
                       )
...
## ---- summary-miRtargets ----
ifileA <- paste0("miRNA_seq/output/miRNA_targets_MF-",
                 i.MF, "_iConf-", i.conf_f, ".txt")
mirFileA <- file.path(supp_data_dir, ifileA)

if (file.exists(mirFileA)) {
    miRtargetsA <- tryCatch(read.table(mirFileA,
                                       comment.char = "#",
                                       header = FALSE),
                           error = function(e) {
                               message(#
    ...
    if (class(miRtargetsA) == "data.frame") {
        ...
        miRtargetsWideA <- reshape(miRtargetsA,
                                   direction  = "wide",
                                   idvar = "ID",
                                   timevar = "V1")
        colnames(miRtargetsWideA) <- mgsub::mgsub(colnames(miRtargetsWideA),
                                                  c("V2.", ":"), c("", ""))
        ...
        alignment.link <- paste0(plot.prefix, '"',
                                 ...
                                 img_file, "'",
                                 plot.suffix_widths,'">',
                                 fa("align-justify", width = icon_size,
                                    height = icon_size,
                                    fill = "steelblue"), "</a>")
...
ifileB <- paste0(
    "miRNA_seq/output/mirmap_miRNA_targets_MF-",
    i.MF, "_iConf-", i.conf_f, ".txt")
mirfileB <- file.path(supp_data_dir, ifileB)

if (file.exists(mirfileB)) {
    miRtargetsB <- tryCatch(read.table(mirfileB,
    ...
    if (class(miRtargetsB) == "data.frame") {
        ...
        miRtargetsWideB <- reshape(miRtargetsB, direction  = "wide",
                                   idvar = "ID", timevar = "V1")
        colnames(miRtargetsWideB) <- mgsub::mgsub(colnames(miRtargetsWideB),
                                                  c("V2.", ":"), c("", ""))
