...
do
	...
	then
	    ...
	    python "${base_dir}/$pydeg_script" \
		   ...
		   -ctrl "$idir_prev/${icomp[1]}"_mapped_genome.sam  \
		   -test "$idir_prev/${icomp[0]}"_mapped_genome.sam \
		   -iconf "${iset[0]}" \
		   -w 4 \
		   -imf "${iset[1]}" \
		   -o "$outfile" &
	...
	then
	    ...
	    python "$base_dir/$pydeg_script" \
		   -gtf "$base_dir/$ref_gtf_sorted" \
		   -ctrl "$idir_prev/${icomp[0]}"_mapped_genome.sam  \
		   -test "$idir_prev/${icomp[1]}"_mapped_genome.sam \
		   -iconf "${iset[0]}" \
		   -w 4 \
		   -imf "${iset[1]}" \
		   -o "$outfile"
...
## 11 - Extract uniquely mapped [Transcript]
# echo "$stp - Extract uniquely mapped [Transcript]"
# stp=$((stp+1))
# idir_prev="06_1-sam_transcript"
# idir="06_2-sam_transcript"
# dir_exist "$idir"
...
#     then
# 	# Sort
# 	samtools sort -@ "$cores"  "$idir_prev/${ifile}" -O sam -o "$idir_prev/${ifile%_mapped_transcriptome.sam}"_sort.sam

# 	# Filter by XS: tag
# 	samtools view -q 10 -G 4 -G 16\
# 		 -h   "$idir_prev/${ifile%_mapped_transcriptome.sam}"_sort.sam | grep  -v "XS:" > "$outfile"
#     fi
# done
...
# stp=$((stp+1))
# idir_prev="06_2-sam_transcript"
# idir="06_3-bam_transcript"
# dir_exist "$idir"
